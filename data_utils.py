...
class OnlinePathLoader:
    ...
    def get_batch(self):
        ...
        while len(pos_path_batch) < self.batch_size:
            ...
            use_memory_prob = 0.5 * len(self.replay_memory[mpid]) / self.memory_size #Quyết định sử dụng dữ liệu từ bộ nhớ hay lấy mẫu mới
            ...
            if pidx < self.topk - 1: #Nếu pidx không phải là vị trí cuối cùng trong top k sản phẩm yêu thích
                neg_pidx = np.random.choice(np.arange(pidx + 1, self.topk)) #Chọn ngẫu nhiên một vị trí neg_pidx trong khoảng từ pidx + 1 đến self.topk
...
class OnlinePathLoaderWithMPSplit:
    ...
    def get_batch(self):
        ...
        while len(pos_path_batch) < self.batch_size:
            ...
            # Sample a negative product.
            if pidx < self.topk - 1: # chọn phần còn lại của top k
